	...
	switch (order)
	{
		...
		case CO_BW+RO_FW+SL_BW:
			...
			this_slice_z = (int)(Z_SUBLEVELS*projection_offset[2]+
				(object_data->slices-1)*slice_z_factor);
			for (this_slice=object_data->slices-1; this_slice>=0; this_slice--)
			{	if (0 && check_event && check_event(this)==FIRST)
				{
					for (tn=0; tn<64827; tn++)
						if (patch[tn])
							free(patch[tn]);
	...
	unsigned short **this_slice_ptr)
{
	...
	if (object_file == NULL)
		return (4);
	if (fseek(object_file,
/*@@*/			(int)(long)object_data->ptr_table[this_slice*object_data->rows], 0))
	{	fclose(object_file);
		return (5);
	}
	*this_slice_ptr = object_data->ptr_table[this_slice*object_data->rows];
	slice_size = object_data->ptr_table[(this_slice+1)*object_data->rows]-
		*this_slice_ptr;
