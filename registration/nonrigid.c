        ...
        int xdim, ydim, zdim;
{
	...
	for(k=0; k<zdim; k++) {
		...
		for(j=0; j<ydim; j++) {
			kj = kk + j*xdim;
		        tmpsum = bcoeff[kj];
		        for(l=1; l<xdim; l++)
		                tmpsum = tmpsum +bcoeff[kj+l]*powf(z1,l);
		        for(l=0; l<xdim-1; l++)
		                tmpsum = tmpsum +bcoeff[kj+l]*powf(z1,2*xdim-2-l);
	...
	for(k=0; k<zdim; k++) {
		...
		for(i=0; i<xdim; i++)
		{
		        tmpsum = bcoeff[kk+i];
		        for(l=1; l<ydim; l++)
		                tmpsum = tmpsum + bcoeff[kk+l*xdim+i]* powf(z1,l);
		        for(l=0; l<ydim-1; l++)
		                tmpsum = tmpsum + bcoeff[kk+l*xdim+i]* powf(z1,2*ydim-2-l);
	...
	for(j=0; j<ydim; j++) {
		...
		for(i=0; i<xdim; i++)
		{
			ji = jj + i;
		        tmpsum = bcoeff[ji];
		        for(l=1; l<zdim; l++)
		                tmpsum = tmpsum + bcoeff[l*llength+ji]*powf(z1,l);
		        for(l=0; l<zdim-1; l++)
		                tmpsum = tmpsum + bcoeff[l*llength+ji]*powf(z1,2*zdim-2-l);
