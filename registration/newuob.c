...
/* newuob.f -- translated by f2c (version 20000121).
...
/* Subroutine */ int newuob_(integer *n, integer *npt, doublereal *x, 
	...
	doublereal *d__, doublereal *vlag, doublereal *w, double (*objective)())
{
    /* Format strings */
...
L490:
	...
	if (*iprint >= 2) {
	    ...
	    printf("New RHO =%.15f     Number of function values = %d\n", rho, nf);
...
/* L540: */
    ...
    if (*iprint >= 1) {
		printf("At the return from NEWUOA     Number of function values =%d\n",nf);
		printf("Function number%6d    F = %.10f\n    The corresponding X is: (",
			(int)nf, f);
		for (i__ = 1; i__ <= *n; ++i__) {
	 	   printf(" %.15f", x[i__]);
