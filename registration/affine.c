...
void setup1(int argc, char *argv[], FILE **in1, FILE **in2)
{
  ...
  *in2 = fopen(argv[3], "rb");
  ...
  if (vh1.scn.domain_valid && vh2.scn.domain_valid)
  {
  ...
  char *argv[];
{
  ...
  if (argc < 2)
  {
    printf("\nUsage 1: affine [s c m n k f] <source> <target> <outfile> <paramfile> [<interpolation>] [<init_mode> 0: rigid, 1: scale, 2: anisotropic scale, 3: volume preserving, 4: affine, 5: homothetic, 6: translation only] [<shift1> <shift2>] [<depth>]\n");
