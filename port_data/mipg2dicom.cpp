...
int main(int argc, char *argv[])
{
    ...
    if (strncmp(argv[argc-1], "-SeriesInstanceUID=", 19) == 0)
    {
        ...
        suid_len = strlen(SeriesInstanceUID);
    ...
    if(argc < 3) {
        printf("Usage:\n");
        printf("mipg2dicom <input-file> <output-file> [<mode> [<first-slice> <last_slice>]] [-SeriesInstanceUID=<UID>]\n");
        printf("input-file : BIM, IM0, or MV0 file name\n");
        printf("output-file : output file name  with no extension \n");
        printf("mode (optional):  0 = foreground 1=background\n");
        exit(1);
    }
      ...
      if (i>=first_slice && i<= last_slice) {
        ...
        if (SeriesInstanceUID || (vh.gen.study_valid && vh.gen.series_valid))
        {
            ...
            put_dcm_data(fpout, 0x0008, 0x0018, AT, (char *)"UI",
                SOPInstanceUID, strlen(SOPInstanceUID));
        }
        if (vh.gen.data_type == IMAGE0)
        ...
        if (vh.gen.slice_thickness_valid) {
            ...
            put_dcm_data(fpout, 0x0018, 0x0050, AN, (char *)"DS", text, strlen(text));
        }

        if (vh.gen.kvp_valid) {
            sprintf(text, "%e", vh.gen.kvp[0]);
            put_dcm_data(fpout, 0x0018, 0x0060, AN, (char *)"DS", text, strlen(text));
        }
        ...
        if (vh.gen.study_valid) {
            ...
            put_dcm_data(fpout, 0x0020, 0x0010, AT, (char *)"SH", text, strlen(text));
        }

        strcpy(text, "1 ");
        put_dcm_data(fpout, 0x0020, 0x0011, AT, (char *)"IS", text, strlen(text));
        sprintf(text,"%d", i+1); /* image number        */
        put_dcm_data(fpout, 0x0020, 0x0012, AT, (char *)"IS", text, strlen(text));
        sprintf(text,"%d", nslices-i); /* acquisition number        */
        put_dcm_data(fpout, 0x0020, 0x0013, AT, (char *)"IS", text, strlen(text));
        if (vh.gen.data_type==IMAGE0)
        {
            sprintf(text, "%e", vh.scn.loc_of_subscenes[vh.scn.dimension==3?i:
                vh.scn.num_of_subscenes[0]+i]); /* slice location */
            put_dcm_data(fpout, 0x0020, 0x1041, AN, (char *)"DS", text, strlen(text));
        }
        ...
        put_dcm_data(fpout, 0x0028, 0x0004, AN, (char *)"CS", text, strlen(text)); 

        ushort_num = 1;
        sprintf(text, "%d", ushort_num);
        put_dcm_data(fpout, 0x0028, 0x0008, AN, (char *)"IS", text, strlen(text));
        ...
        if (vh.gen.data_type == IMAGE0) {
            ...
            put_dcm_data(fpout, 0x0028, 0x0030, AN, (char *)"DS", text, strlen(text));
        } else {
        ...
        if (vh.gen.data_type == IMAGE0) {
            ...
            if (vh.scn.axis_label_valid) {
                ...
                put_dcm_data(fpout, 0x0028, 0x1100, AN, (char *)"CS", text, strlen(text));
            }
        }
        ...
        if ((vh.gen.data_type == MOVIE0) &&  (num_bits == 8)) {
           ...
           ni = fwrite (out_data, 1, data_size, fpout);
            ...
            for (j=0; j < image_size; j++) 
              for (k=0; k < 8; k++) { 
                if (8*j+k > data_size)
                  ...
                  ni= fwrite(&uc_data_1, 1, 1, fpout);
                else 
                  ni= fwrite(&uc_data_0, 1, 1, fpout);
                check_IO (ni == 1); 
              }
          } else
#endif
          if (num_bits == 16) {
             ...
             ni = fwrite (data, 1, image_size, fpout);
	...
	char *vr, char *str, unsigned int len)
{
  ...
  fprintf( stderr, "(group, element)=(%X, %X) (%d): ", us1, us2, len ); 
  for (int i=0; i<(int)len; i++) {
     if (isprint(str[i]))    fprintf( stderr, "%c",   str[i] );
     else                    fprintf( stderr, "(%d)", str[i] );
  }
  fprintf( stderr, "\n" );  fflush( stderr );
