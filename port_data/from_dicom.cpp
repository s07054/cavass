...
int rec_arch_type; /* 0, 1, 2 or 3, depending on the architecture of the ACR-NEMA receiver */
...
int main(int argc, char *argv[])
{
	...
	int rename_dicom=0;
	int slope_flag=0;
	float max_slope=0, slope_unit;
	int move_dicom_subset=0;


	bi = 0x100;
	bigendian = *((unsigned char *)&bi);
	...
	if(argc < 3)
	{
		fprintf(stderr, "Usage: from_dicom [ -l <input_list_file> | <input_file> ... ] <output_file> [-p] [-t] [+<add_value>] [-rename] [-slopex<unit>] [-use-acquisition] [-move_dicom_subset <subset_scene>]\n");
		exit(1);
	}
	...
	/* 'rec_arch_type' is a global used by 'get_element()'. Also checked to perform SWAP of image bytes */
	rec_arch_type = 0; /* this is a global used by functions in "read_acrnema.c" */
	skip = 0;
	...
	/* Gather information about each series */
	{
		...
		for (dir_entry=0; dir_entry<totslices; dir_entry++)
		{
			...
			if( a > 0 &&  b == 0)
			{
				...
				nslices[j]++;
				nframes[j] += get_num_frames(cur_filename);
			}
		}
		for(i=0; i<nseries; i++)
		{
	...
	/* <<< General >>> */
	/*******************************************************************************/
	if (get_element(fpin, 2, 0x10, AT, uid, 64, &items_read)==0 &&
