...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void VOIROIFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void VOIROIFrame::OnBrowseScreen ( wxCommandEvent& unused ) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void VOIROIFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
...
BEGIN_EVENT_TABLE       ( VOIROIFrame, wxFrame )
  ...
  EVT_BUTTON( ID_APPEND_SCREEN,    VOIROIFrame::OnAppendScreen    )
  EVT_BUTTON( ID_BROWSE_SCREEN,    VOIROIFrame::OnBrowseScreen    )
  EVT_BUTTON( ID_OVERWRITE_SCREEN, VOIROIFrame::OnOverwriteScreen )
