...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SurfViewFrame::OnSaveScreen ( wxCommandEvent& e ) {
...
void SurfViewFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}

void SurfViewFrame::OnBrowseScreen ( wxCommandEvent& unused ) {
...
void SurfViewFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
...
void run_command(const char cmnd[], bool fg)
{
	ProcessManager( "Running command...", cmnd, fg );
}
