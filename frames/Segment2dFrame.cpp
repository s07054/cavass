...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/** \brief append the current canvas contents to the current save screen
 *  file.
 *  \todo  this needs to be implemented.
 */
void Segment2dFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/** \brief overwrite the current save screen file with the contents of
 *  the canvas.
 *  \todo  this needs to be implemented.
 */
void Segment2dFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
...
void Segment2dFrame::OnBrushSize( wxCommandEvent& e )
{
	Segment2dCanvas *canvas = dynamic_cast<Segment2dCanvas*>(mCanvas);
	if (canvas->detection_mode == Segment2dCanvas::TRAINING)
		canvas->train_brush_size = atoi((const char *)e.GetString().c_str());
	else
		canvas->paint_brush_size = atoi((const char *)e.GetString().c_str());
}
