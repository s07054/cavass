...
void CycleCanvas::OnRightDown ( wxMouseEvent& e ) {
	...
	//remove all of the measure points
	if (mf->mWhichMode == CycleFrame::ModeMeasure) {
		mf->mWhichMode = CycleFrame::ModeLayout;
		mf->mMode->SetSelection( CycleFrame::ModeLayout );
		wxCommandEvent  ce;
		mf->OnMode( ce );  //arg is not used
		mf->removeAll();
        if (mf->mTextControls!=NULL) {
            delete mf->mTextControls;
            mf->mTextControls = NULL;
        }
