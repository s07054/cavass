...
void FuzzCompFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}

void FuzzCompFrame::OnBrowseScreen ( wxCommandEvent& unused ) {
...
void FuzzCompFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
...
//@@
  ...
  switch (choice2) {
      case HARD:
		 break;
      case FUZZY:
         s5 += wxString::Format( " -fuzzy-adjacency");
         break;
  }
...
DefineStandardFrameCallbacks
  ...
  EVT_BUTTON( ID_APPEND_SCREEN,    FuzzCompFrame::OnAppendScreen    )
  EVT_BUTTON( ID_BROWSE_SCREEN,    FuzzCompFrame::OnBrowseScreen    )
  EVT_BUTTON( ID_OVERWRITE_SCREEN, FuzzCompFrame::OnOverwriteScreen )
