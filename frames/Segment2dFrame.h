 ...
 */
//======================================================================
#ifndef __Segment2dFrame_h
#define __Segment2dFrame_h
 ...
 *  slice from a data set.
 <p>
  ...
  This example consists of the following files:
    Segment2dFrame.cpp,  Segment2dFrame.h, Segment2dCanvas.cpp, and Segment2dCanvas.h.
 ...
 <p>
  ...
  Montage files are:
      ...
      MontagePrint.h.
  \todo These other modules need to be updated to be in accord with the Segment2d app.
 </p>
 <p>
 ...
 <ol>
  ...
  <li> Add the following to MainFrame.cpp (this is the callback function that
       ...
       Using Segment2dFrame::createSegment2dFrame() in this manner allows one 
...
class Segment2dFrame : public MainFrame {
  protected:
	...
	SetSegment2dOutputControls *mSetOutputControls;
	Segment2dAuxControls *mAuxControls;
  public:
  ...
  protected:
	...
	void deleteSegment2dControls ( void );
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  public:
    static void createSegment2dFrame ( wxFrame* parentFrame, bool useHistory=true );
    Segment2dFrame ( bool maximize=false, int w=800, int h=600 );
    ~Segment2dFrame ( void );
...
extern char Segment2dFeatureNames[6][15];
extern char Segment2dTransformNames[6][16];
...
public:
	Segment2dAuxControls( wxPanel* cp, wxSizer* bottomSizer,
		const char * const title, int currentBrushSize, bool update_flag=false)
	{
		setup_fgs(cp, bottomSizer, title, 2);
		static const char sa[][3]={"X", "1", "2", "5", "10", "15", "20", "25",
			"30", "35", "40", "50", "60"};
		int ii=0;
		assert(currentBrushSize <= atoi(sa[sizeof(sa)/sizeof(sa[0])-1]));
		if (currentBrushSize)
		...
		double newFeatureMin, double newFeatureMax)
	{
		...
		switch (newTransform) {
			case 0: // Linear
			case 2: // Inv. Linear
				mFeatureMin = new Segment2dSlider( mCp, mFgsSliders, "Feature Min.",
					Segment2dFrame::ID_FEATURE_MIN, 0, featureRange,
					newFeatureMin );
				mFeatureMax = new Segment2dSlider( mCp, mFgsSliders, "Feature Max.",
			...
			case 5: // Inv. Hyperbolic
				mMean = new Segment2dSlider( mCp, mFgsSliders, "Mean",
					Segment2dFrame::ID_MEAN, 0, featureRange, newFeatureMean );
				mStdDev = new Segment2dSlider( mCp, mFgsSliders, "Std. Dev.",
		...
		double newFeatureMin, double newFeatureMax)
	{
		...
		if (mWeight)
			delete mWeight, mWeight = NULL;
		mWeight = new Segment2dSlider( mCp, mFgsSliders, "Weight",
		...
		mTransform->SetValue(Segment2dTransformNames[newFeatureTransform]);
	}
