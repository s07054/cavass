/*
  Copyright 1993-2008, 2016 Medical Image Processing Group
 ...
 */
//======================================================================
#ifndef __CycleCanvas_h
#define __CycleCanvas_h
...
/** \brief CycleCanvas - the canvas on which images and other things
 *  are drawn (i.e., the drawing area of the window).
 */
class CycleCanvas : public MainCanvas {
public:
...
protected:
    ...
    bool  mDataInitialized;  ///< data has been initialized flag
    ...
    public:
            ...
            int px2, int py2, int pz2, int value2 )
        {
            ...
            mDistance = sqrt( (mX1-mX2)*(mX1-mX2) + (mY1-mY2)*(mY1-mY2) +
                              (mZ1-mZ2)*(mZ1-mZ2) );
        }
  ...
  public:
    CycleCanvas ( void );
    CycleCanvas ( wxWindow* parent, MainFrame* parent_frame, wxWindowID id,
  ...
  public:
    ...
    void reload ( void );
    bool mapWindowToData ( int wx, int wy, const CycleData* const which, int& d, int& x, int& y, int& z );
    ...
    inline void mapDataToWindow ( int x, int y, int z, int& wx, int& wy ) const
    {
        ...
        wx = wy = -1;
        //if (!inBounds(x, y, z)) {
        //    wxLogMessage( "CycleCanvas::mapDataToWindow: out of bounds at (%d,%d,%d).", x, y, z );
        //}
        if      (x<0)         x=0;
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    DECLARE_DYNAMIC_CLASS( CycleCanvas )
    DECLARE_EVENT_TABLE()
};
