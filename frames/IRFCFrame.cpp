...
void IRFCFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}

void IRFCFrame::OnBrowseScreen ( wxCommandEvent& unused ) {
...
void IRFCFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
...
DefineStandardFrameCallbacks
  ...
  EVT_BUTTON( ID_APPEND_SCREEN,    IRFCFrame::OnAppendScreen    )
  EVT_BUTTON( ID_BROWSE_SCREEN,    IRFCFrame::OnBrowseScreen    )
  EVT_BUTTON( ID_OVERWRITE_SCREEN, IRFCFrame::OnOverwriteScreen )
