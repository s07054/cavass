...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void VOIIOIFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void VOIIOIFrame::OnBrowseScreen ( wxCommandEvent& unused ) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void VOIIOIFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
...
BEGIN_EVENT_TABLE       ( VOIIOIFrame, wxFrame )
  ...
  EVT_BUTTON( ID_APPEND_SCREEN,    VOIIOIFrame::OnAppendScreen    )
  EVT_BUTTON( ID_BROWSE_SCREEN,    VOIIOIFrame::OnBrowseScreen    )
  EVT_BUTTON( ID_OVERWRITE_SCREEN, VOIIOIFrame::OnOverwriteScreen )
