...
void Segment2dCanvas::reload ( void ) {
	...
	else
	{
		...
		if (mCols>1 && mRows)
		{
			Calc_Combined_Edge_Costs(temp_list, 0);
...
void Segment2dCanvas::OnMouseMove ( wxMouseEvent& e ) {
	...
	if (e.LeftIsDown() && !e.ShiftDown() && !e.ControlDown() && layout_flag)
	{
		...
		wy >= orig.locy && wy < orig.locy+orig.h)
	{
		...
		switch (detection_mode)
		{
			...
			case LSNAKE:
				...
				break;
			case PAINT:
				if (e.MiddleIsDown() && phase==0 && paint_brush_size)
					erase_mask(wx, wy);
...
void Segment2dCanvas::OnRightDown ( wxMouseEvent& e ) {
    ...
    if (e.AltDown() || e.ControlDown() || e.MetaDown() || e.ShiftDown())
        OnMiddleDown( e );
...
void Segment2dCanvas::OnLeftDown ( wxMouseEvent& e ) {
    ...
    if (e.ShiftDown()) {
        //pass the event up to the parent frame
        Segment2dFrame*   p = (Segment2dFrame*)m_parent_frame;
...
*/
	...
	SumCost = OUTSIDE_MASK - (Imax-Imin) - 1.0;
...
printf("Open greymap.TMP L8597\n");
	...
	strcpy(mother_file, "///");
	strncpy(mother_file+4, (const char *)m_fname.GetFullName().c_str(), 100);
