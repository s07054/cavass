...
void InterpolateFrame::addButtonBox ( void ) {
    ...
    s = wxString::Format( "%d", mCanvas->mCavassData->m_zSize );
    ...
    if (nDim==4) {
        ...
        mX4Value = new wxTextCtrl( mControlPanel, ID_X4_VALUE, "" );
        ::setColor( mX4Value );
...
void InterpolateFrame::OnSave ( wxCommandEvent& unused ) {
    ...
    //perform the command
    assert( mCanvas->mCavassData->getNumberOfDimensions() == 3 );  /** \todo add 4D support */
