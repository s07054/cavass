...
static void usage ( char* programName ) {
	fprintf( stderr, "\nUsage: \n%s [X Y Z] outputImageFile \n    X Y Z = the size of the output (XxYxZ) (default=%dx%dx%d) \n",
		programName, dXSize, dYSize, dZSize );
  exit( EXIT_FAILURE );
}
//----------------------------------------------------------------------
int main ( int argc, char* argv[] ) {
    ...
    BoxMuller  b;
    for (int i=0; i<dN; i++) {
        int  x = (int)(b.sample( dXSize/2.0, dXSize/10.0 ) + 0.5);
        int  y = (int)(b.sample( dYSize/2.0, dYSize/20.0 ) + 0.5);
        int  z = (int)(b.sample( dZSize/2.0, dZSize/20.0 ) + 0.5);
		if (inBounds(x,y,z)) {
			assert( x>0 && y>0 && z>0 );
			assert( x<dXSize-1 && y<dYSize-1 && z<dZSize-1 );
			int  sub = z*dXSize*dYSize + y*dXSize + x;
            if (data[sub] == dMin) {  //not set yet?
    ...
    printf( "%d attempts set %d unique points. \n", dN, count );
#else
    for (int z=dSize/2-10; z<=dSize/2+10; z++) {
        for (int y=dSize/2-10; y<=dSize/2+10; y++) {
            for (int x=dSize/2-10; x<=dSize/2+10; x++) {
                data[z][y][x] = dMax;
                printf( "(%d,%d,%d) \n", x, y, z );
            }
        }
