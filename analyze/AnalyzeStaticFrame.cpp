...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void AnalyzeStaticFrame::OnOverwriteScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "overwrite screen file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
//  EVT_BUTTON( ID_LAYOUT,         AnalyzeStaticFrame::OnLayout        )
  ...
  EVT_BUTTON( ID_APPEND_SCREEN,    AnalyzeStaticFrame::OnAppendScreen    )
  EVT_BUTTON( ID_BROWSE_SCREEN,    AnalyzeStaticFrame::OnBrowseScreen    )
  EVT_BUTTON( ID_OVERWRITE_SCREEN, AnalyzeStaticFrame::OnOverwriteScreen )
