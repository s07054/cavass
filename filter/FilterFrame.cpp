...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#if 0
void FilterFrame::OnAppendScreen ( wxCommandEvent& unused ) {
    wxString  s = wxString::Format( "append screen to file %s",
        (const char *)mSaveScreenControls->getFileName().c_str() );
    SetStatusText( s,  0 );
}
#endif
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#if 0
void FilterFrame::OnBrowseScreen ( wxCommandEvent& unused ) {
    wxString  filename = wxFileSelector( _T("Select image file"), _T(""),
        _T(""), _T(""),
        "CAVASS screen files (*.scrn)|*.scrn" );
...
void FilterFrame::OnFilterSave ( wxCommandEvent &e ) 
{
	...
	cmd1 += Preferences::getHome()+"/";
	cmd2 += Preferences::getHome()+"/";
	cmd3 += Preferences::getHome()+"/";
	int iterations=1;
    ...
    switch (m_FilterType) 
	{
	  ...
	  case FILTER_DILATE:
		  ...
		  {
		      cmd1 += wxString::Format("morph\" \"%s\" \"%s\" %d %d",
			      (mCanvas->mCavassData)->m_fname,
				  temp_file2.GetFullPath().c_str(), canvas->getMorphN(), 0);
			  cmd2 += wxString::Format("morph\" \"%s\" \"%s\" %d %d",
			      temp_file1.GetFullPath().c_str(),
				  temp_file2.GetFullPath().c_str(), canvas->getMorphN(), 0);
			  cmd3 += wxString::Format("morph\" \"%s\" \"%s\" %d %d",
			      temp_file1.GetFullPath().c_str(),
				  (const char *)saveDlg.GetPath().c_str(),
				  canvas->getMorphN(), 0);
		  }
	...
	{
		wxLogMessage( "command=%s", (const char *)cmd1.c_str() );
		ProcessManager  p( "filter running...", cmd1, !mBGProcess );
		unlink((const char *)temp_file1.GetFullPath().c_str());
		rename((const char *)temp_file2.GetFullPath().c_str(),
			(const char *)temp_file1.GetFullPath().c_str());
		for (int j=2; j<iterations; j++)
		{
			wxLogMessage( "command=%s", (const char *)cmd2.c_str() );
			p = ProcessManager( "filter running...", cmd2, !mBGProcess );
			unlink((const char *)temp_file1.GetFullPath().c_str());
			rename((const char *)temp_file2.GetFullPath().c_str(),
			    (const char *)temp_file1.GetFullPath().c_str());
		}
		wxLogMessage( "command=%s", (const char *)cmd3.c_str() );
		p = ProcessManager( "filter running...", cmd3, !mBGProcess );
		unlink((const char *)temp_file1.GetFullPath().c_str());
	}
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
BEGIN_EVENT_TABLE       ( FilterFrame, wxFrame )
  ...
  EVT_BUTTON( ID_APPEND_SCREEN,    FilterFrame::OnAppendScreen    )
  EVT_BUTTON( ID_BROWSE_SCREEN,    FilterFrame::OnBrowseScreen    )
