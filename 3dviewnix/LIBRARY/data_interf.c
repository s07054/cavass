...
static int v_WriteHeader_10 ( FILE* fp, ViewnixHeader* vh )
{
        ...
        switch (vh->gen.data_type) {
            ...
            case IMAGE1 : /* scene of type IMAGE1 */
                error=v_read_items("1.0",0) ;
                if (error != 0) return(error) ;
                error=v_write_general_10(fp,&(vh->gen), (long*)&offset,&grplen,&items,
                                       &error1) ;
                if (error != 0) return(error) ;
                error=v_write_scene_10(fp,&(vh->scn), (long*)&offset,&grplen,&items,
                                     &error1);
                if (error != 0) return(error) ;
                error=v_write_len(fp, (long*)&offset,&grplen,structure_grp) ;
                if (error != 0) return(error) ;
                error=v_write_len(fp, (long*)&offset,&grplen,display_grp) ;
                if (error != 0) return(error) ;
                break ;
            case CURVE0 : /* structure date of type CURVE0 */
            case SURFACE0: /* structure data of type SURFACE0 */
            ...
            case SHELL2: /* structure data of type SHELL2 */
                ...
                error=v_write_general_10(fp,&(vh->gen), (long*)&offset,&grplen,&items,
                                       &error1) ;
                if (error != 0) return(error) ;
                error=v_write_len(fp, (long*)&offset,&grplen,scene_grp) ;
                if (error != 0) return(error) ;
                error=v_write_structure_10(fp,&(vh->str), (long*)&offset,
                        &grplen,&items,&error1);
                if (error != 0) return(error) ;
                error=v_write_len(fp, (long*)&offset,&grplen,display_grp) ;
                if (error != 0) return(error) ;
            ...
            case MOVIE0 :  /* display data of type MOVIE0 */
                ...
                error=v_write_len(fp, (long*)&offset,&grplen,structure_grp) ;
                if (error != 0) return(error) ;
                error=v_write_display_10(fp,&(vh->dsp), (long*)&offset,&grplen,&items,
                                       &error1) ;
                if (error != 0) return(error) ;
                break ;
            default :
    ...
    int* items, int* error1 )
{
        ...
        skip=32 ;
        i= *items ;
        if (gen->recognition_code == NULL)
            v_print_fatal_error("VWriteHeader",
                "gen->recognition_code should not be NULL.",
                0) ;
...
{ 
        ...
        error=v_short_to_file(fp,scn->bytes_in_alignment,
                            ...
                            &acr_items[i++],error1) ;
        if (error != 0) {if(label)free(label); return(error);}
        if (scn->xysize == NULL && scn->xysize_valid == 1) 
            v_print_fatal_error("VWriteHeader",
                "scn->xysize should not be NULL.",0) ;
        ...
        if (scn->xypixsz_valid == 1 && scn->xypixsz == NULL)
            v_print_fatal_error("VWriteHeader",
                "scn->xypixsz should not be NULL.",0) ;
        ...
        if (str->dimension_valid == 1) {
            ...
            if (str->num_of_samples_valid == 1) 
                ...
                if (error != 0) return(error) ;
                        ...
                        /**002B 8120****/
        if (str->xysize_valid == 1 && str->xysize == NULL) 
            v_print_fatal_error("VWriteHeader",
                    "str->xysize should not be NULL.",0) ;
...
/**************end changes***************************/
        ...
        error=v_short_to_file(fp,dsp->num_of_images,dsp->num_of_images_valid,
                            &grplen,&acr_items[i++],error1) ;
        if (error != 0) return(error) ;
        if (dsp->xysize_valid == 1 && dsp->xysize == NULL)
            v_print_fatal_error("VWriteHeader",
                 "dsp->xysize should not be NULL.",0) ;
        error=v_shorts_to_file(fp,dsp->xysize,dsp->xysize_valid,2,&grplen,
                             &acr_items[i++],error1) ;
        if (error != 0) return(error) ;
        if (dsp->xypixsz_valid == 1 && dsp->xypixsz == NULL)
            v_print_fatal_error("VWriteHeader",
                 "dsp->xypixsz should not be NULL.",0) ;
...
int v_ReadData ( char* data, int size, int items, FILE* fp )
{
  ...
  items_read=fread(data,size,items,fp);
...
#define SHORT_BLOCK BLOCK_SIZE/2
  ...
  switch (size) {
  case 1:  /* char - no conversion to be made */
    return(fwrite(data,size,items,fp));
  case 2:  /* short - two possible coversions */
    if ( *short_test == 0x0100 ) { /* bytes_switched */
      items_written=0;
      while (items_written+SHORT_BLOCK<=items) {
        ...
        cur_written=fwrite(block,size,SHORT_BLOCK,fp);
      ...
      return(items_written + fwrite(block,size,left_over,fp));
    }
    else 
      return(fwrite(data,size,items,fp));
    break;
  case 4: /* int - 4 possible convertons */
    ...
    case 0x03020100:
      ...
      while (items_written+INT_BLOCK<=items) {
        ...
        cur_written=fwrite(block,size,INT_BLOCK,fp);
        items_written+=cur_written;
