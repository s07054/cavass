...
static void wrap_up(), next_slice(), hollow_voxel(), init_row(), next_row(),
	init_scene(int argc, char **argv),
 get_gradient(float *gxp, float *gyp, float *gzp, data_value *previous_slice,
    data_value *current_slice, data_value *following_slice, int sliceno),
 z_interpolate(), read_slice(), get_slice(), vol_exit(int code);
...
static void read_slice()
{
	int j;

	if (byte_in_slice == NULL)
	{	if (VReadData((char *)in_slice1, in_slice_size/(dtcolumns*dtrows),
				dtcolumns*dtrows, dtfile, &j))
		{	(void)fprintf(stderr, "Input error %sat %d\n",
				feof(dtfile)? "EOF ": "", (int)ftell(dtfile));
			vol_exit(2);
		}
	}
	else
	{	if (VReadData((char *)byte_in_slice, in_slice_size/(dtcolumns*dtrows),
				dtcolumns*dtrows, dtfile, &j))
		{	(void)fprintf(stderr, "Input error %sat %d\n",
				feof(dtfile)? "EOF ": "", (int)ftell(dtfile));
			vol_exit(2);
...
static void get_gradient(float *gxp, float *gyp, float *gzp,
    data_value *previous_slice, data_value *current_slice,
	data_value *following_slice, int sliceno)
{
	...
	mb = (float)(
		(p[-1][0][0]+1/SQRT2*(p[-1][1][0]+p[-1][-1][0]+p[-1][0][1]+
		p[-1][0][-1])+1/SQRT3*(p[-1][1][1]+p[-1][-1][1]+p[-1][1][-1]+
		p[-1][-1][-1]))*(1/(1+2*SQRT2+4/SQRT3)));
	mf = (float)(
		(p[1][0][0]+1/SQRT2*(p[1][1][0]+p[1][-1][0]+p[1][0][1]+p[1][0][-1])+
		1/SQRT3*(p[1][1][1]+p[1][-1][1]+p[1][1][-1]+p[1][-1][-1]))*
		(1/(1+2*SQRT2+4/SQRT3)));
	*gxp = (float)(.5*(mf-mb)/voxel_size);
	mb = (float)(
		(p[0][-1][0]+1/SQRT2*(p[0][-1][-1]+p[0][-1][1]+p[-1][-1][0]+
		p[1][-1][0])+1/SQRT3*(p[-1][-1][-1]+p[1][-1][-1]+p[-1][-1][1]+
		p[1][-1][1]))*(1/(1+2*SQRT2+4/SQRT3)));
	mf = (float)(
		(p[0][1][0]+1/SQRT2*(p[0][1][-1]+p[0][1][1]+p[-1][1][0]+p[1][1][0])+
		1/SQRT3*(p[-1][1][-1]+p[1][1][-1]+p[-1][1][1]+p[1][1][1]))*
		(1/(1+2*SQRT2+4/SQRT3)));
	*gyp = (float)(.5*(mf-mb)/voxel_size);
	mb = (float)(
		(p[0][0][-1]+1/SQRT2*(p[0][-1][-1]+p[0][1][-1]+p[-1][0][-1]+
		p[1][0][-1])+1/SQRT3*(p[-1][-1][-1]+p[1][-1][-1]+p[-1][1][-1]+
		p[1][1][-1]))*(1/(1+2*SQRT2+4/SQRT3)));
	mf = (float)(
		(p[0][0][1]+1/SQRT2*(p[0][-1][1]+p[0][1][1]+p[-1][0][1]+p[1][0][1])+
		1/SQRT3*(p[-1][-1][1]+p[1][-1][1]+p[-1][1][1]+p[1][1][1]))*
		(1/(1+2*SQRT2+4/SQRT3)));
	*gzp = (float)(.5*(mf-mb)/slice_thickness);
}
...
-------------------*/
			...
			next_slice_opacity_ptr->pct = (unsigned char)(percentage*255);
			break;
		...
		case GRADIENT:
			if (*next_slice_in_ptr >= min_density)
			{
				...
				gradient = (float)sqrt(gx*gx+gy*gy+gz*gz);
				...
				{	float grad_byte;
					...
					next_slice_opacity_ptr->gradient =
						(unsigned char)grad_byte;
					...
					next_slice_opacity_ptr->opacity =
						(unsigned char)(density*grad_byte);
				}
				else
...
/* Modified: 9/20/07 output_file_name set by Dewey Odhner. */
static void init_scene(int argc, char **argv)
{
	...
	if (argc == 21)
	{
		output_file_name = argv[20];
		strncpy(file_header.gen.filename, argv[20],
	...
	strncpy(file_header.gen.filename1, argv[1]+j,
		sizeof(file_header.gen.filename1)-1);
	file_header.gen.filename1[sizeof(file_header.gen.filename1)-1] = 0;
	max_gradient = (float)atof(argv[2]);
	min_opacity = (int)(atof(argv[3])*255);
	max_opacity = (int)(atof(argv[4])*255);
	threshold[0] = (float)atof(argv[5]);
	threshold[1] = (float)atof(argv[6]);
	threshold[2] = (float)atof(argv[7]);
	threshold[3] = (float)atof(argv[8]);
	threshold[4] = (float)atof(argv[9]);
	threshold[5] = (float)atof(argv[10]);
	opn[0] = (float)atof(argv[11]);
	opn[1] = (float)atof(argv[12]);
	opn[2] = (float)atof(argv[13]);
	opn[3] = (float)atof(argv[14]);
	first_slice = atoi(argv[15]);
	final_slice = atoi(argv[16]);
	voxel_size = (float)atof(argv[17]);
	slice_thickness = (float)atof(argv[18]);
	nbits = atoi(argv[19]);
	for (k=0; k<256; k++)
	{	for (j=1; j<4; j++)
			materl_opacity[2*j][k]= (unsigned char)(k*opn[j]+(255-k)*opn[j-1]);
		for (j=0; j<4; j++)
			materl_opacity[2*j+1][k] = (unsigned char)(255*opn[j]);
	}
	...
	if ((dtfile = fopen(argv[1],"rb")) == NULL ) {
			printf("Can't open %s\n",argv[1]);
			vol_exit(1);
	}
 ...
 *    tempfile, tempfile_name, output_file_name,
...
static void wrap_up()
{
	...
	*this_ptr_ptr = last_out_ptr;
	...
	file_header.str.largest_value[1] = (float)max_x;
	file_header.str.largest_value[2] = 7;
	file_header.str.largest_value[3] = 6;
	file_header.str.largest_value[4] = (float)((1<<nbits)-1);
	file_header.str.largest_value[5] = (float)((1<<nbits)-1);
	...
	shell_file = fopen(output_file_name, "w+b");
	if (shell_file == NULL)
	...
	error_code = VWriteData((char *)tree_data, 2, file_header.str.num_of_NTSE[0],
	...
	{	if (fread(voxel_data, 2, last_out_ptr, tempfile) !=
		...
		error_code = VWriteData((char *)voxel_data, 2, 3*file_header.str.num_of_TSE[0],
			shell_file, &items_written);
		...
		for (j=0; j<(int)file_header.str.num_of_TSE[0]; j++)
		{	if (fread(voxel_datum, 2, 3, tempfile) != 3)
			...
			error_code =
				VWriteData((char *)voxel_datum, 2, 3, shell_file, &items_written);
