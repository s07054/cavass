  ...
  if (vh.scn.dimension==4) {
    ...
    for(i=0;i<vols;i++)
      for(j=0;j<slices-1;j++)
        if ( fabs(Pz- fabs(vh.scn.loc_of_subscenes[vols+i*slices+j+1] -
                           vh.scn.loc_of_subscenes[vols+i*slices+j])) > 0.001) {
    ...
    for(j=0;j<slices-1;j++)
      if ( fabs(Pz- fabs(vh.scn.loc_of_subscenes[j+1] -
                         vh.scn.loc_of_subscenes[j])) > 0.001) {
        printf("Cannot handle data with unequal slices spacing\n");
        fflush(stdout);
        exit(-1);
      }
	...
	int force_domain, float domain[12])
{
  ...
  if (ND_FLAG) {
    ...
    /* tranform origin */
    s2->domain[0]= (float)(orig[0]*s1->domain[3]+orig[1]*s1->domain[4]+
      orig[2]*s1->domain[5] + s1->domain[0]);
    s2->domain[1]=(float)(orig[0]*s1->domain[6]+orig[1]*s1->domain[7]+
      orig[2]*s1->domain[8] + s1->domain[1]);
    s2->domain[2]=(float)(orig[0]*s1->domain[9]+orig[1]*s1->domain[10]+
      orig[2]*s1->domain[11] + s1->domain[2]);

    /* tranform x_axis */
    s2->domain[3]= (float)(x_axis[0]*s1->domain[3]+x_axis[1]*s1->domain[4]+
      x_axis[2]*s1->domain[5]);
    s2->domain[4]=(float)(x_axis[0]*s1->domain[6]+x_axis[1]*s1->domain[7]+
      x_axis[2]*s1->domain[8]);
    s2->domain[5]=(float)(x_axis[0]*s1->domain[9]+x_axis[1]*s1->domain[10]+
      x_axis[2]*s1->domain[11]);

    /* tranform y_axis */
    s2->domain[6]= (float)(y_axis[0]*s1->domain[3]+y_axis[1]*s1->domain[4]+
      y_axis[2]*s1->domain[5]);
    s2->domain[7]=(float)(y_axis[0]*s1->domain[6]+y_axis[1]*s1->domain[7]+
      y_axis[2]*s1->domain[8]);
    s2->domain[8]=(float)(y_axis[0]*s1->domain[9]+y_axis[1]*s1->domain[10]+
      y_axis[2]*s1->domain[11]);

    /* tranform z_axis */
    s2->domain[9]= (float)(z_axis[0]*s1->domain[3]+z_axis[1]*s1->domain[4]+
      z_axis[2]*s1->domain[5]);
    s2->domain[10]=(float)(z_axis[0]*s1->domain[6]+z_axis[1]*s1->domain[7]+
      z_axis[2]*s1->domain[8]);
    s2->domain[11]=(float)(z_axis[0]*s1->domain[9]+z_axis[1]*s1->domain[10]+
        ...
        for(i=0;i<plane_size[1];i++)
          for(j=0;j<plane_size[0];j++,map++,img_ptr++) 
            if (*map==n) { /* if the lower slice index matches n */
              ...
              x=Floor(point[0]);
              if (x>=0  && x<vh->scn.xysize[0]-1) {
                point[1]= cur_ends[0][1] + i*v[1] + j*u[1] ;
                y=Floor(point[1]);
                if (y>=0  && y<vh->scn.xysize[1]-1) {
                  ...
                  *img_ptr= (unsigned short)(interp? .5+ 
                    (1-dz)*
                      ( (1-dy)*( (1-dx)* *pt1 +
				                     dx* *(pt1+1) ) +
                           dy *( (1-dx)* *(pt1+vh->scn.xysize[0]) +
					                 dx* *(pt1+vh->scn.xysize[0]+1) )
                      )+
                       dz *
				      ( (1-dy)*( (1-dx)* *(pt2) +
				                     dx* *(pt2+1) ) +
                           dy *( (1-dx)* *(pt2+vh->scn.xysize[0]) +
				   ...
				   :
					dz<.5?
					  dy<.5?
					    dx<.5? *pt1: *(pt1+1)
					  :
					    dx<.5? *(pt1+vh->scn.xysize[0])
						: *(pt1+vh->scn.xysize[0]+1)
					:
					  dy<.5?
					    dx<.5? *pt2: *(pt2+1)
					  :
					    dx<.5? *(pt2+vh->scn.xysize[0])
						: *(pt2+vh->scn.xysize[0]+1));
                }
			  }
            }
       }
                ...
                if (y>=0 && y<vh->scn.xysize[1]-1) {
                  ...
                  *img_ptr_8= (unsigned char)(interp? .5+ 
                    (1-dz)*
                      ( (1-dy)*( (1-dx)* *pt1_8 +
			 	                     dx* *(pt1_8+1) ) +
                        dy    *( (1-dx)* *(pt1_8+vh->scn.xysize[0]) +
			 	 	                 dx* *(pt1_8+vh->scn.xysize[0]+1) )
                      )+
                     dz *
                      ( (1-dy)*( (1-dx)* *(pt2_8) +
			 	                     dx* *(pt2_8+1) ) +
                        dy    *( (1-dx)* *(pt2_8+vh->scn.xysize[0]) +
				   ...
				   :
					dz<.5?
					  dy<.5?
					    dx<.5? *pt1_8: *(pt1_8+1)
					  :
					    dx<.5? *(pt1_8+vh->scn.xysize[0])
						: *(pt1_8+vh->scn.xysize[0]+1)
					:
					  dy<.5?
					    dx<.5? *pt2_8: *(pt2_8+1)
					  :
					    dx<.5? *(pt2_8+vh->scn.xysize[0])
						: *(pt2_8+vh->scn.xysize[0]+1));
                }
              }
		    }
      } 
