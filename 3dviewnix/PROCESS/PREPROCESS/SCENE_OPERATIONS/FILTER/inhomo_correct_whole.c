...
extern int compute_scale_image(char *o_file, char *s_file, double a, int b),
 GenerateLabels(char *dt, int w, int h, int s),
 get_factor_nD(voxelElem *pElem, long int mask_volume, double factor[]),
	GenerateLabels_new(char *dt, int w, int h, int s);
void compute_scale(int largest_density_value),
 bin_to_grey(unsigned char *bin_buffer, int length, unsigned char *grey_buffer,
...
double VOLUME_THRESHOLD=0.0001; 
...
int inhomo_correct(char *o_file, char *b_file, char *c_file, char *i_file, int W);

int number_largest_object,num_pre=0;

/********************find nn*****************************************************/
int find_nn(char *o_file, char *s_file, double a, double b);
/**************************************for compute scale*******************************/
#define FILTER  1
#define SCALE 12
...
/*****************************************************************************/
char *datafile[FEATURES], *outfile[FEATURES];
static int Way=0,times=10;
double NN1=1.0,NN2=1.0;
int METHOD=0,ii_temp=0;
/**************************************************************************************/
void usage()
{
  printf("\n");
  printf("Usage: inhomo_correct_whole <ori_file> <corrected_file> -mode [1 | 0] [-left <left>] [-right <right>] [-times <times>] [-c18] [-c26] [-volume_threshold <v_th>] [-stop_condition <stop>]\n");
  printf("-mode      : multiplicative 1 or additive 0\n");
  printf("<left>     : mean-left*sigma\n");
  printf("<right>    : mean+right*sigma\n");
  printf("<times>    : iteration number\n");
  printf("<v_th>     : default %f\n", VOLUME_THRESHOLD);
  printf("<stop>     : default %f\n", STOP_CONDITION);
...
char *argv[];
{
  ...
  while (args_parsed < argc)
    {    
      if (strcmp(argv[args_parsed], "-left") == 0)
        {
          args_parsed++;
          sscanf(argv[args_parsed], "%lf", &NN1);
          args_parsed++;
        }
      else
        {
          if (strcmp(argv[args_parsed], "-right") == 0)
            {
              args_parsed++;
              sscanf(argv[args_parsed], "%lf", &NN2);
              args_parsed++;
            }
          else
            {
              if (strcmp(argv[args_parsed], "-times") == 0)
                {
                  args_parsed++;
                ...
                if (strcmp(argv[args_parsed], "-volume_threshold") == 0)
                  {
                    args_parsed++;
                    sscanf(argv[args_parsed], "%lf", &VOLUME_THRESHOLD);
                    args_parsed++;
                  }
                else
                  if (strcmp(argv[args_parsed], "-stop_condition") == 0)
                    {
                      args_parsed++;
                      sscanf(argv[args_parsed], "%lf", &STOP_CONDITION);
                      args_parsed++;
                    }
                  else
                    {
                      if (!strcmp(argv[args_parsed],"-c18"))
                        CONNECTIVITY=18;
                      else if (!strcmp(argv[args_parsed],"-c26"))
                        CONNECTIVITY=26;
                    }
            }
        }
...
char *argv[];
{
  ...
  if (copy_file(datafile[0], "temp_ori.IM0"))
    {
  ...
  for(ii=0;ii<times;ii++)
    {      
      ...
      printf("Computing scale .......\n\n");
      temp=compute_scale_image("temp_ori.IM0","temp_scale.IM0",1.0,0);
      printf("scale finished.......\n\n");
      printf("Finding nn.......\n");

      temp=find_nn("temp_ori.IM0", "temp_scale.IM0", NN1, NN2); 
      if (temp!=0)
        {
          printf("Error occur!\n");
          exit(-1);
        }
      printf("Finding nn finished.......\n");
      printf("Inhomo correction.......\n");
      temp=inhomo_correct("temp_ori.IM0","temp_bin.BIM","temp_cor.IM0","temp_inhomo.IM0",Way);
      if (temp==0)
        {
      ...
      if ((temp==2)&&(ii==0))
	{
	  printf("\nObject is too small! Please using larger left and right value.\n");
	  break;
	}
      
      if ((temp==2)&&(ii>0))
	{
	  printf("Inhomo correction finished.\n");
	  break;
	}
      
      num_pre=number_largest_object;
      if ((temp==1))
        if (copy_file("temp_cor.IM0", "temp_ori.IM0"))
        {
  ...
  if ((temp==0) || ((temp==2)&&(ii>0)) || (temp==1) )
    {
      unlink(outfile[0]);
      rename("temp_cor.IM0", outfile[0]);
      unlink("temp_inhomo.IM0");
    }
  
  unlink("temp_scale.IM0");
 ...
 *    Modified: 12/5/01 (double)2. passed to pow by Dewey Odhner.
 *    Modified: 12/6/01 unsigned values cast to int for subtraction
 *       by Dewey Odhner.
 *
 *******************************************************************************************/
int compute_scale_image(char *o_file, char *s_file, double a, int b)
{
  int i,j,k,l,error_code, count, **histogram, hist_sum[FEATURES], feature_thr[FEATURES];
  int tti1,tti2,min,max;
  int x,y,z,xx,yy,zz,largest_density_value, ***ppptti1;
  double tt1,tt2;
  static ViewnixHeader vh_in,vh_out;
  char group[6],element[6],*datafile[FEATURES],*outfile;
  ...
  for (yy = -FILTER; yy <= FILTER; yy++)
    for (xx = -FILTER; xx <= FILTER; xx++)
      {
        tt1 = anisotropy_col * xx;
        tt2 = anisotropy_row * yy;
        tt2 = 1 / (1 + tt1*tt1 + tt2*tt2);
  ...
  /*----to compute the histogram for each feature and the threshold for true edge---*/
      ...
      count = 0;
        ...
        scale_map = (float *) malloc( (largest_density_value + 2) * sizeof(float));
        
        //------------------ GAUSSIAN ----------------------------------------------------
        inv_scale_sigma = -0.5 / pow(homogeneity_sigma, 2.0);
        for (i = 0; i <= largest_density_value+1; i++)
          scale_map[i] = (float)exp(inv_scale_sigma * pow((double) i, 2.0));
    }
  
  compute_scale(largest_density_value);
  ...
  for(i = 0;i<pslice;i++)
    for(j = 0;j<prow;j++)
      for(k = 0;k<pcol;k++)
        {
          tti1 = scale_image[i*slice_size + j*pcol + k];
          if(tti1 < min ) min = tti1;
          if(tti1 > max ) max = tti1;
        }
  ...
  switch (error_code)
  {
    ...
    case 106:
    case 107:
      if (fwrite(scale_image, 1, volume_size, fp_out) == volume_size)
        break;
    default:
 ...
 *    Modified: 12/11/01 rounding error reduced by Dewey Odhner.
 *
 *****************************************************************************/
void compute_scale(int largest_density_value)
{
  int i, k, x, y, z, xx, yy, tti5, slice, row, col;
  int flag, tti1;
  double count_obj, count_nonobj, tt1, tt2, tt3, mean_g;
                  ...
                  for (xx = -FILTER; xx <= FILTER; xx++)  {
                    ...
                    if (x >= 0 && y >= 0 
                      ...
                      tt3 = tt3 + mask[yy + FILTER][xx + FILTER] 
                        * (double) data_scale8[z * slice_size + y * pcol + x];
                    else
                      tt3 = tt3 + mask[yy + FILTER][xx + FILTER] 
                        * (double) data_scale8[slice * slice_size + row * pcol + col];
                  }
                mean_g = tt3 / mask_total;
                ...
                for (k = 1; k < SCALE && !flag; k++)  {
                  ...
                  for (i = 0; i < sphere_no_points[k]; i++)  {
                    ...
                    if (tt2 < 0)
                      tt2 = -tt2;
                    tti5 = (int)tt2;
                    assert(tti5 <= largest_density_value);
                    count_obj = count_obj +
                      ((1+tti5-tt2)*scale_map[tti5] + (tt2-tti5)*scale_map[tti5+1]);
                    ...
                    for (xx = -FILTER; xx <= FILTER; xx++)
                      {
                        ...
                        if (x >= 0 && y >= 0  
                            && x < pcol && y < prow)
                          tt3 = tt3 + mask[yy + FILTER][xx + FILTER] 
                            * (double) data_scale16[z * slice_size + y * pcol + x];
                        else
                          tt3 = tt3 + mask[yy + FILTER][xx + FILTER] 
                            * (double) data_scale16[slice * slice_size+ row * pcol + col];
                      }
                  mean_g = tt3 / mask_total;
 ...
 *       by Dewey Odhner.
 *    Modified: 12/4/01 dt_object initialized by Dewey Odhner.
 *    Modified: 12/12/01 data_cor initialized by Dewey Odhner.
 *
 *******************************************************************************************/
int inhomo_correct(o_file,b_file,c_file,i_file,W)
char *o_file,*b_file,*c_file,*i_file;
int W;
{
  int i,j,k,error1,tti1,tti2,tti3,ii,jj, iteration;
  int xx,yy,zz;
  double tt1,mean_value[FEATURES], mean[10];
  ...
  unsigned char  **data8;
  unsigned short *data_cor;
  unsigned short *inhomo_int;
  float *inhomo_intensity;
  double **bV,**cV,**dV,**dir;
  long int volume=0;
  char *dt_object;
  double ori_min,ori_max;
  ...
  ori_min=vh.scn.smallest_density_value[0];
  ori_max=vh.scn.largest_density_value[0];
    
  if(num_of_bits == 8)
    {
      data8 = (unsigned char **)malloc(FEATURES*sizeof(unsigned char*));
      data8[0] = (unsigned char *)malloc(FEATURES*volume_size*sizeof(char));
      ...
      for (i = 0;i< FEATURES;i++)
        {
          ...
          VSeekData(in, 0);
          VReadData((char *)data8[i], 1, volume_size, in, &j);    
    ...
    {
      data16 = (unsigned short **)malloc(FEATURES*sizeof(unsigned short *));
      data16[0] = (unsigned short *)malloc(FEATURES*volume_size*sizeof(unsigned short));
      ...
      memset(data16[0],0,2*volume_size);
      ...
      for (i = 0;i< FEATURES;i++)
        {
          ...
          error1=VReadData((char *)data16[i], 2, volume_size, in, &j);    
  ...
  if(num_sets>0)
    {
      ...
      memcpy(pSort, pRegion, num_sets*sizeof(*pSort));
      ...
      if (abs(number_largest_object-num_pre)>num_pre*STOP_CONDITION)
        {
          //compute average intensity value of the each region
                  ...
                  for(k=0;k<tti1;k++)
                    {
                      ...
                      tti2 = zz*slice_size + yy*width + xx;
                      if(num_of_bits == 8)
                        sum = sum + data8[j][tti2];
                      else if (num_of_bits == 16)
                        sum = sum + data16[j][tti2];
                      pLocation++;
                    }
                  mean_value[j] = (double)(sum) /(double)(tti1);
                  mean[i]=mean_value[j];
                  printf("mean of the object is: %f\n",mean_value[j]);   
                }
                  ...
                  for(j=0;j<FEATURES;j++)
                    {
                      if ((num_of_bits == 8)&&(W==0))
                        tt1 = tt1*((double)data8[j][tti2]-(double)mean_value[j]);
                      if ((num_of_bits == 8)&&(W==1))
                        tt1 = tt1*((double)data8[j][tti2]/(double)mean_value[j]);
                      if ((num_of_bits == 16)&&(W==0))
                        tt1 = tt1*((double)data16[j][tti2]-(double)mean_value[j]);
              ...
              if (Way==0)
                {
                  ...
                  vh1.scn.largest_density_value[0] = (float)(int)(fmax-fmin);
                }
              else
                {
                  vh1.scn.smallest_density_value[0] = 50 ;
                  vh1.scn.largest_density_value[0] = (float)(int)(fmax-fmin+50);
                }
              
            }
          else
            {
              vh1.scn.smallest_density_value[0] =(float)(int)(fmin);
              vh1.scn.largest_density_value[0] = (float)(int)fmax;
            }
          ...
          for(i=0;i<volume_size;i++)
            inhomo_int[i]=(int)rint(inhomo_intensity[i]); 
          
          VWriteData((char *)inhomo_int,sizeof(short),volume_size,out,&j);   
          free(inhomo_int); 
          fclose(out);
          
          if (num_of_bits==8)
            {
              data_cor=(unsigned short *)malloc(volume_size*sizeof(short));
              if (data_cor==NULL)
                {
                  printf("Error is in mallocing!\n");
                  exit(-1);
                }
            }
          ...
          for(i = 0;i<FEATURES;i++)
            {
              if (num_of_bits==8)
                for(zz = 0; zz<volume_size; zz++)
                  data_cor[zz] = data8[i][zz];
              out=fopen(c_file,"wb+");
              if (out==NULL) 
                {
                  ...
                  for(xx = 0; xx<width; xx++)
                    {
                      ...
                      if ((num_of_bits==8)&&(W==1)&&(inhomo_intensity[tti1] != 0))
                        {
                          tt1=100.0*(double)data8[i][tti1]/(double)inhomo_intensity[tti1];
                          data_cor[tti1]=(int)(tt1);
                          
                        }
                      if ((num_of_bits==8)&&(W==0))
                        {
                          tt1 = data8[i][tti1] - inhomo_intensity[tti1];
                          data_cor[tti1]=(int)(tt1);
                          
                        }             
                      if ((num_of_bits == 16)&&(W==1)&&(inhomo_intensity[tti1] != 0))
                        {
                          tt1 = 100.0*(double)data16[i][tti1]/(double)inhomo_intensity[tti1];
                          data16[i][tti1] =(int)(tt1);
                        }             
                      if ((num_of_bits==16)&&(W==0))
                        {
		  ...
		  for(xx = 0; xx<width; xx++)
		    {
		      tti1 = zz*height*width + yy*width + xx;
		      if(num_of_bits==8)
			data_cor[tti1]= (unsigned short)(ori_min+((float)(data_cor[tti1]-fmin)/(fmax-fmin))*(float)(ori_max-ori_min));
		      else if(num_of_bits == 16)
			data16[i][tti1] = (unsigned short)(ori_min+((float)(data16[i][tti1] - fmin)/(fmax-fmin))*(float)(ori_max-ori_min));
		    }
              vh1.scn.smallest_density_value[0] =(float)(int)(ori_min) ;
	      vh1.scn.largest_density_value[0] = (float)(int)(ori_max);
	      vh1.scn.largest_density_value_valid = 1;
              vh1.scn.smallest_density_value_valid = 1;
              vh1.scn.num_of_bits=16; 
              if (vh1.scn.bit_fields_valid) vh1.scn.bit_fields[1] =15; 
              ...
              if(num_of_bits == 8)
                VWriteData((char *)data_cor,sizeof(unsigned short),volume_size,out,&j);
              else if(num_of_bits == 16)
                VWriteData((char *)data16[i],sizeof(short),volume_size,out,&j);
              VCloseData(out);
            }
          for(i = 0;i<FEATURES;i++)
            if(num_of_bits == 8)
...
/********************************************************************************************
 * FUNCTION: find_nn
 * DESCRIPTION: find the best inteval about the most homogeneity part 
 * PARAMETERS: original image, scale image of the original image and constant NN1, NN2
 *     
 * SIDE EFFECTS: None
 * ENTRY CONDITIONS: None
 * RETURN VALUE: 
 ...
 * EXIT CONDITIONS: None
 * HISTORY:
 *    Created: 07/07/00 by Jiamin Liu
 *    Modified: 12/5/01 (double)2. passed to pow by Dewey Odhner.
 *    Modified: 1/22/02 variable sum changed to double by Jiamin Liu.
 *******************************************************************************************/
int find_nn(o_file,s_file,a,b)
char *o_file,*s_file;
double a,b;
{
  int i,j,k,l, error_code, x,y,z,xx,yy,zz,largest_value;
  static ViewnixHeader vh_in;
  char group[6],element[6];
  double sum;
  double mean,std;
  int t1,t2,tti1,SCALE_MAX,kk,tti2, ***ppptti1;
  double tt1,anisotropy_col,anisotropy_row,anisotropy_slice;
  char *datafile1,*datafile2, *command;
  double std_sum,mean_sum;
  int num_of_bits;
  FILE *fp_in1, *fp_in2;
  unsigned char *data1_8, *data2;     
  unsigned short *data1_16;
  regionElem *pTemp,swap;
  voxelElem *pLocation;
  int num_largest_scale_region;
  long int tti_number=0;

  datafile1 = o_file;
  datafile2 = s_file;
   
  fp_in1 = fopen(datafile1, "rb");
  ...
  fp_in2 = fopen(datafile2, "rb");
  error_code = VReadHeader(fp_in2, &vh_in, group, element);
  switch (error_code)
        {
        case 0:
        case 106:
        case 107:
          break;
        default:
          fprintf(stderr, "file = %s; group = %s; element = %s\n", datafile2, group, element);
        }
  fclose(fp_in2);
  
  SCALE_MAX=(int)vh_in.scn.largest_density_value[0];
  ...
  sphere_no_points = (int *) malloc((SCALE_MAX + 1) * sizeof(int));
  sphere_points = (void *) malloc((SCALE_MAX + 1) * sizeof(void *));
  ...
  if (tt1 > anisotropy_row)
    tt1 = anisotropy_row;
  if (slices > 1 && tt1 > anisotropy_slice)
    tt1 = anisotropy_slice;
  anisotropy_col = anisotropy_col / tt1;
  anisotropy_row = anisotropy_row / tt1;
  if (slices > 1)
      ...
      for (k = 0; k <= SCALE_MAX; k++)
        {
            ...
            for (l = -k - 2; l <= k + 2; l++)
              {
                  ...
                  if (tt1 <= ((double) k) + 0.5)
                    {
                      sphere_no_points[k] = sphere_no_points[k] + 1;
                      ppptti1[tti1][tti1 + j][tti1 + l] = 2; //test
                    }
                }
          ...
          for (j = -k - 2; j <= k + 2; j++)
            for (l = -k - 2; l <= k + 2; l++)
              if (ppptti1[tti1][tti1 + j][tti1 + l] == 2)  //test
                {
  ...
  if(num_of_bits == 8)
    {
      data1_8 = (unsigned char *)malloc(volume_size*sizeof(char));
      memset(data1_8,0,volume_size*sizeof(char));
      if (data1_8 == NULL) 
        {
          printf("Memory allocation error \n");
          exit(-1);
    ...
    {
      data1_16 = (unsigned short *)malloc(volume_size*sizeof(short));
      memset(data1_16,0,volume_size*sizeof(short));
      if (data1_16 == NULL) 
        {
          printf("Memory allocation error \n");
          exit(-1);
  ...
  data2=(unsigned char *)malloc((volume_size)*sizeof(char));
  ...
  if (num_of_bits==8)
    { 
      error_code = VReadData((char *)data1_8, num_of_bits/8, volume_size, fp_in1, &j);
      error_code = VReadData((char *)data2, 1, volume_size, fp_in2, &j);
    }
  else if (num_of_bits==16)
    {
      error_code = VReadData((char *)data1_16, num_of_bits/8, volume_size, fp_in1, &j);
      error_code = VReadData((char *)data2, 1, volume_size, fp_in2, &j);
    }  
  ...
  if (num_of_bits==8)
    {
      for(i=0;i<volume_size;i++)
        if (data1_8[i]>largest_value)
          largest_value=data1_8[i];
    }
  else
    {
      for(i=0;i<volume_size;i++)
        if (data1_16[i]>largest_value)
          largest_value=data1_16[i];
    }
  
  for(i=0;i<volume_size;i++)
    if (data2[i]==SCALE_MAX)
      dt_8[i]=1;
    else
      dt_8[i]=0;
  ...
  /* Build Multiplication tables */
  tbz=(unsigned int *)malloc(sizeof(int)*slices);
  tby=(unsigned int *)malloc(sizeof(int)*vh_in.scn.xysize[1]);
  if (tbz==NULL || tby==NULL) 
    {
      fprintf(stderr,"Error in allocating data\n");
      exit(-1);
    }
  for(j=vh_in.scn.xysize[0],i=0;i<vh_in.scn.xysize[1];i++)
    tby[i]=i*j;
  
  for(j=vh_in.scn.xysize[0]*vh_in.scn.xysize[1],i=0;i<slices;i++)
    tbz[i]=i*j;
  ...
  /* Generate the actual labels */
  // return total regions number and total voxels number
  
  num_largest_scale_region = GenerateLabels_new((char *)dt_8,vh_in.scn.xysize[0],vh_in.scn.xysize[1],slices);
  ...
  pSort = (regionElem *)malloc(sizeof(regionElem)*num_largest_scale_region);
  if(pSort == NULL)
    {
      fprintf(stderr,"Error in allocating data\n");
      exit(-1);
  ...
  pTemp = pSort; 
  for(i = 0;i<num_largest_scale_region;i++)
    {
      *pTemp = *pRegion;
      pTemp++;
      pRegion++;
    }
  free(pRegion-num_largest_scale_region);

  for(i = 0;i<num_largest_scale_region-1;i++)
    {
      k = i;
      for(j = i+1;j<num_largest_scale_region;j++)
        if(pSort[j].num_voxels > pSort[k].num_voxels)
          k = j;
      swap = pSort[i];
      pSort[i] = pSort[k];
  ...
  if (tti1>volume_size*SCALE_THRESHOLD)
    {
      ...
      for(k=0;k<tti1-5;)
        {
          ...
          sum = 0;
          for (i = 0; i < sphere_no_points[kk]; i++)
            {
              x = xx + sphere_points[kk][i][2];
              y = yy + sphere_points[kk][i][1];
              z = zz + sphere_points[kk][i][0];
              if (x < 0 || x >= width)
                x = xx;
              if (y < 0 || y >= height)
                y = yy;
              ...
              if(num_of_bits == 8)
                sum += data1_8[z * slice_size + y * width + x];
              else if (num_of_bits == 16)
                sum += data1_16[z * slice_size + y * width + x];
            }
          
          mean = (double)sum /sphere_no_points[kk];
          sum=0;
          for (i = 0; i < sphere_no_points[kk]; i++)
            {
              x = xx + sphere_points[kk][i][2];
              ...
              if(num_of_bits == 8)
                sum += pow((data1_8[z * slice_size + y * width + x]-mean),2.);
              else if (num_of_bits == 16)
                sum += pow((data1_16[z * slice_size + y * width + x]-mean),2.);
            }
          std = sqrt((double)sum/sphere_no_points[kk]);
          pLocation=pLocation+5;
          mean_sum=mean_sum+mean;
          std_sum=std_sum+std;
          k=k+5;
          tti_number=tti_number+1;
        }
      mean = (double)mean_sum/tti_number;
      std = (double)std_sum/tti_number;
      printf("mean of the object is: %f\n",mean);        
      printf("std of the object is: %f\n",std); 
    }
  else
     {
      kk=SCALE_MAX;
      ...
      mean = (double)mean_sum/tti1;
      std = (double)std_sum/tti1;
      printf("mean of the object is: %f\n",mean);        
      printf("std of the object is: %f\n",std); 
  ...
  command = (char *)malloc(60+strlen(datafile1));
  sprintf(command,"ndthreshold %s temp_ori.BIM 0 %d %d",datafile1,t1,t2);
  system(command);
  free(command);

  rename("temp_ori.BIM", "temp_bin.BIM");
...
/********************************************************************************************
 * FUNCTION: GenerateLabels_new
 * DESCRIPTION: find the connected object after thresholding the scale image.
 * PARAMETERS: the array after thresholding the scale image, width, hight and slices about the
 *             original image
 * SIDE EFFECTS: None
 * ENTRY CONDITIONS: None
 * RETURN VALUE: the number of connected objects
 *  
 * EXIT CONDITIONS: None
 * HISTORY:
 ...
 *
 *******************************************************************************************/
int GenerateLabels_new(char *dt, int w, int h, int s)
{
  short x,y,z;
  int i;
  int volume;
          ...
          if (*pt==1)
            {
              ...
              volume = FindObject(dt,w,h,s,x,y,z); 
...
void PushQ(char *d, short x, short y, short z, int* total_num)
{
   ...
   *total_num =  *total_num + 1;
   ...
   assert(tot_dt < volume_size);
   pGlobal[tot_dt++] = voxel;
   Q[push].x=x;
   Q[push].y=y; 
   Q[push].z=z; 
   push++;
   assert(push < volume_size);
 }
...
int copy_file(const char *src, const char *dst)
{
	...
	while (!feof(fpin))
	{
		bytes_just_read = fread(buffer, 1, CP_BUFFER_SIZE, fpin);
		if (ferror(fpin))
		{
			fclose(fpin);
			fclose(fpout);
		...
		bytes_just_writ = fwrite(buffer, 1, bytes_just_read, fpout);
