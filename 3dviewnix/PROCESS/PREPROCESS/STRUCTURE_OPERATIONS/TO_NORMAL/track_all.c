...
char *argv[];
{
  ...
  char grp[6],elem[6],*ptr,exec_str[1150],tempname[400];
  ...
  strncpy(in.filename, argv[1]+j, sizeof(in.filename)-1);
  ...
  sprintf(med_filename,"TRACK3D_%d.TEMP",getpid());
  if ((med_fp=fopen(med_filename,"w+b"))==NULL) {
    printf("Could not open temp file  for computations\n");
    exit(-1);
  }
  ...
  strncpy(out.filename, argv[2], sizeof(out.filename)-1);
  ...
  if (merge_flag) {
    if ((out.fp=fopen(out.filename,"rb"))==NULL) {
      ...
      fclose(out.fp);
      sprintf(tempname,"T%s",out.tempname);
